function [current_spec_lin_new, velocity] = read_nc_LV0__VelocityFold_Ze_normalize(current_spec_lin, range_offsets, len_alias, specN ,range ,maxVel  )
...
% 1) Observed linear Ze to Normalized linear Ze; 2) Unfold velocity when the velocity exceeds Nyquist velocity (maxV)    
    ...
    if specN(Num_chirp) < 1024 % patch nan to make the same length
        current_spec_lin_new2(:,specN(Num_chirp)+1:1024) = nan;
    end
    tempv = -maxVel(Num_chirp) : maxVel(Num_chirp)*2/single(specN(Num_chirp)-1) :maxVel(Num_chirp);
    d_v = tempv(2) - tempv(1) ;
    ...
    current_spec_lin_new3 =  factor_ze * [temp  current_spec_lin(range_offsets(Num_chirp)+1: length(range),   ...
        1: specN(Num_chirp)-len_alias)] ;   
    if specN(Num_chirp) < 1024 % patch nan to make the same length
        current_spec_lin_new3(:,specN(Num_chirp)+1:1024) = nan;
    end
    tempv = -maxVel(Num_chirp) : maxVel(Num_chirp)*2/single(specN(Num_chirp)-1) :maxVel(Num_chirp);
    d_v = tempv(2) - tempv(1) ;
