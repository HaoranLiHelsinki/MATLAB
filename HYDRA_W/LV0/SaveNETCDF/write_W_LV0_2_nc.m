function write_W_LV0_2_nc(data , header ,outfile, config)
...
did_len_spec = netcdf.defDim(ncid,'len_spec', max(header.SpecN) );
...
id_AntiAlias = netcdf.defVar(ncid,'AntiAlias','nc_float', did_scalar);
...
id_Vmin = netcdf.defVar(ncid,'MinVel','nc_float',[did_time,did_range]);
...
id_Aliasmask = netcdf.defVar(ncid,'AliasMask','nc_float',[did_time,did_range]);
...
netcdf.putVar(ncid,id_Aliasmask,[0,0],[data.TotSamp,header.NumbGates],single(data.Aliasmask) );
netcdf.putVar(ncid,id_Vmin,[0,0],[data.TotSamp,header.NumbGates],data.MinVel);

netcdf.putVar(ncid,id_spec_V,[0,0,0],[data.TotSamp,header.NumbGates , max(header.SpecN)],data.spec);
netcdf.putVar(ncid,id_noise_V,[0,0],[data.TotSamp,header.NumbGates],data.VNoisePow_mean);

if header.DualPol > 0    
    netcdf.putVar(ncid,id_spec_HV,[0,0,0],[data.TotSamp,header.NumbGates, max(header.SpecN)],data.spec_hv);
