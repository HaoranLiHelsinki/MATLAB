...
% Read spectra data for LV0 Version 3.5 
% Haoran Li haoran.li@helsinki.fi 
% 25th Sep, 2019
...
if header.CompEna == 0 
    ...
    data.spec(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN; % spectra
    if header.DualPol > 0
        data.spec_hv(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN; % spectra 
        data.spec_covRe(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN; % spectra 
        data.spec_covIm(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN; % spectra 
    end

else %  header.CompEna > 0 
  
        try
            data.spec(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
        catch
             if feof(fid)
        ...
        if header.DualPol > 0
            try
                data.spec_hv(data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
                data.spec_covRe(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
                data.spec_covIm(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
            catch
                disp('nanwhat?')
                if feof(fid)
                    endoffileerrormessage
        ...
        if header.CompEna == 2
            data.d_spec(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
            data.CorrCoeff(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
            data.DiffPh(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) =  NaN;

            if header.DualPol == 2
                data.SLDR(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN ;
                data.SCorrCoeff(1:data.TotSamp, 1:header.NumbGates,1:header.SpecN(chirp_idx)) = NaN;
            end 
        end 
...
for indx = 1:data.TotSamp
    ...
    for indxR = 1:header.NumbGates
        ...
        chirp_idx = int32(find(header.RngOffs(2:end) - indxR > 0,1,'first'));
